...
class WeatherViewController: UIViewController, CLLocationManagerDelegate, ChangeCityDelegate {
    ...
    let APP_ID = "a3a75400f4a64d79bc2c6d60fad5185c"
    ...
    //MARK: - JSON Parsing
     ...
     func updateWeatherData(json: JSON)
     {
         ...
         weatherDataModel.temperature = Int(tempResult - 273.15)    //Force unwrapping so as to get a double and thereby converting to Int
    ...
    func updateUIWithWeatherData()
    {
        ...
        temperatureLabel.text = "\(weatherDataModel.temperature)°"  //"\()" means converting to string
        weatherIcon.image = UIImage(named: weatherDataModel.weatherIconName)
        labelText.text = temp
        
        if temperatureLabel.text! <= "29°" && temperatureLabel.text! >= "25°"
        {
            labelText.text = "Wheat "
        }
        if temperatureLabel.text! <= "30°" && temperatureLabel.text! >= "26°"
        {
            labelText.text = "Rice "
        }
        if temperatureLabel.text! <= "32°" && temperatureLabel.text! >= "27°"
        {
            labelText.text = "Cotton "
        }
        if temperatureLabel.text! <= "30°" && temperatureLabel.text! >= "20°"
        {
            labelText.text = "Potato "
        }
        if temperatureLabel.text! <= "30°" && temperatureLabel.text! >= "20°"
        {
            labelText.text = "Banana "
        }
        if temperatureLabel.text! <= "30°" && temperatureLabel.text! >= "20°"
        {
            labelText.text = "Tea "
        }
        if temperatureLabel.text! <= "28°" && temperatureLabel.text! >= "15°"
        {
            labelText.text = "Coffee "
        }
        if temperatureLabel.text! <= "27°" && temperatureLabel.text! >= "18°"
        {
            labelText.text = "Maize "
        }
        if temperatureLabel.text! <= "32°" && temperatureLabel.text! >= "27°"
        {
            labelText.text = "Millets "
        }
        if temperatureLabel.text! <= "25°" && temperatureLabel.text! >= "20°"
        {
            labelText.text = "Grams "
        }
        if temperatureLabel.text! <= "27°" && temperatureLabel.text! >= "21°"
        {
            labelText.text = "Sugarcane "
        }
        if temperatureLabel.text! <= "30°" && temperatureLabel.text! >= "20°"
        {
            labelText.text = "Oilseeds "
        }
    }
    ...
    /* @IBAction func getCropsByWeather(_ sender: UIButton) {
        
        self.performSegue(withIdentifier: "getCropsByWeather", sender: self)
